...
?>
          <div class="salon_wrapper salon_wrapper_2">
                ...
                <div class="">
                   ...
                   <div class="col-sm-12">
                      <div class="right_side_salon">
                         ...
                         <a href="saloon_details.php?id=<?php echo $sln['id'];?> target="_blank">
...
var iconBase = 'image/location.png';
      ...
      var icn =[
       <?php 
$expersds=mysqli_query($conn,"select * from users where role='2' ");

while($rowda = mysqli_fetch_array($expersds)){

    $offercountqsds=mysqli_num_rows(mysqli_query($conn,"select * from promotion where salon_id='".$rowda['id']."' and status='0'"));
                    if($offercountqsds >0){
...
<?php } } }?>
 ...
 var labels =[
         <?php 
$xxx=mysqli_query($conn,"select * from users where role='2' ");

while($rowdaa = mysqli_fetch_array($xxx)){


  $offercountqss=mysqli_num_rows(mysqli_query($conn,"select * from promotion where salon_id='".$rowdaa['id']."' and status='0'"));
                    if($offercountqss >0){
...
<?php } } }?>
  ...
  var locations = [
 <?php 
$exper=mysqli_query($conn,"select * from users where role='2' ");


 while($row = mysqli_fetch_array($exper)){
  $offercountqs=mysqli_num_rows(mysqli_query($conn,"select * from promotion where salon_id='".$row['id']."' and status='0'"));
                    if($offercountqs >0){
   ?>
...
<?php 
$expers=mysqli_query($conn,"select * from users where role='2'");
...
$offercountq=mysqli_num_rows(mysqli_query($conn,"select * from promotion where salon_id='".$rowd['id']."' and status='0'"));
                    if($offercountq >0){
...
?>
'<div style="width:100%"><a href="saloon_details.php?id=<?php echo $rowd['id'];?>" target="_blank"><img src="image/<?php echo $rowd['salon_pic'];?>" style="width: auto;max-width: 90px;"><h1 style="font-size: 12px;    margin: 13px 0 0;font-weight: bold;"><?php echo $rowd['business_name'];?></h1><h1 style="font-size: 12px;    margin: 13px 0 0;font-weight: bold;"><?php if($countsr==0){}else{echo round($reviews,'1');echo "&nbsp;";
...
<script type="text/javascript">
  ...
  function showPosition(position) {
    ...
    window.location="promotionsalon.php?lat="+clatt+"&long="+clongi;
